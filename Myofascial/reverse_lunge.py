...
class ReverseLungeAnalyzer:
    def __init__(self):
        ...
        self.feedback = "Stand with feet hip-width. Step back to lunge."
    ...
    def draw_premium_ui(self, frame):
        ...
        cv2.putText(frame, "REVERSE LUNGE ANALYSIS", (40, 50), cv2.FONT_HERSHEY_DUPLEX, 0.8, self.colors['text'], 2)
    ...
    def process_landmarks(self, landmarks, w, h):
        """Process pose landmarks for reverse lunge analysis"""
        def get_coords(idx):
            lm = landmarks[idx]
        ...
        # In a deep lunge, both knees are bent, usually < 140 deg
        ...
        # Pass (knee, ankle) to get vector pointing DOWN, aligning with (0,1) reference for 0 deg vertical
        l_shin_vert = self.calculate_vertical_angle(l_knee, l_ankle)
        r_shin_vert = self.calculate_vertical_angle(r_knee, r_ankle)
        
        # The one with smaller vertical angle is likely the front leg
        ...
        if is_lunging:
            ...
            if abs(torso_angle) > 20:
            ...
            elif front_shin_vert > 20: 
                self.feedback = "Watch front knee! Keep it over ankle."
                ...
                self.feedback = "Perfect depth! Drive up through front heel."
    ...
    def save_report(self, frame):
        ...
        cv2.putText(report, "REVERSE LUNGE POSTURE ANALYSIS REPORT", (w // 2 - 280, h + 50), 
...
def main():
    ...
    analyzer = ReverseLungeAnalyzer()

    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)

    print("--- Reverse Lunge Analysis Started ---")
    ...
    while cap.isOpened():
        ...
        cv2.imshow('Premium Reverse Lunge Analysis', processed_frame)
        ...
        # Additional check in case waitKey is skipped or returns -1
        if cv2.getWindowProperty('Premium Reverse Lunge Analysis', 0) < 0:
             break
